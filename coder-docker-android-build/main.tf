...
# ---------------------------------------------------------------------------
# Build-only: no emulator, so resource ceilings are modest. Gradle still likes
# a few cores and 4GB+.
...
data "coder_parameter" "memory" {
  ...
  description  = "Amount of memory in GB (4+ recommended for Gradle)"
  default      = "4"
  type         = "number"
  icon         = "/icon/memory.svg"
  mutable      = true
...
resource "coder_agent" "main" {
  ...
  startup_script          = <<-EOT
    ...
    echo "Build-only Android workspace. Build with: ./gradlew assembleDebug  (or: flutter build apk)"
    echo "To test on hardware, connect a networked device: adb connect <device-ip>:5555"
...
# Build the image from ./build/Dockerfile so the Android SDK, Flutter, Node +
# Claude CLI are baked in.
