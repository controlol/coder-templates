...
resource "coder_agent" "main" {
  ...
  startup_script          = <<-EOT
    ...
    # --- Start the in-workspace Docker daemon (DinD) ----------------------
    # The container runs privileged (see main.tf). dockerd runs as root in the
    # background; the `coder` user is in the `docker` group so the socket is
    # usable without sudo once it's up.
    if ! docker info >/dev/null 2>&1; then
      sudo mkdir -p /var/lib/docker
      sudo dockerd --group docker >/tmp/dockerd.log 2>&1 &
...
# Build the image from ./build/Dockerfile so Docker CE + image tooling + Claude
# CLI are baked in.
...
# Persistent /var/lib/docker — keeps built images & layer cache across restarts
# (and off the home volume, which the user's files live on).
...
resource "docker_container" "workspace" {
  ...
  # Required for Docker-in-Docker: the in-workspace dockerd needs full host
  # capabilities (mounts, cgroups, iptables, overlayfs).
  privileged = true
